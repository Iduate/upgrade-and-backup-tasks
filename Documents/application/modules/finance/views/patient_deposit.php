    ...
    <section class="wrapper site-min-height">
        ...
        <section class="no-print col-md-8">
            <header class="panel-heading">
                ...
                <div class="panel-body no-print pull-right">
                    <a data-toggle="modal" href="#myModalb">
                        <div class="btn-group">
                            <button id="" class="btn btn-xs green">
                                Clear
                            </button>
...
<div class="modal fade" id="myModalb" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header no-print">
                <button type="button" class="close no-print" data-dismiss="modal" aria-hidden="true">×</button>
                <h4 class="modal-title"> Clear Patient</h4>
            </div>
            <div class="modal-body clearfix">
                <div class="panel panel-primary">
                    <small>Patient will be able to perform hospital operations even without paying their bills for a period of time</small>
                
                    <form role="form" action="finance/clearPatient" class="clearfix" method="post">
                        ...
                        <input type="hidden" name="p_id" value="<?=$this->input->get('patient')?>"/>
...
<script type="text/javascript">
                            $(document).ready(function () {
                                $(".editbutton").click(function (e) {
                                    e.preventDefault(e);
                                    $('#myModal').find('[class="modal-title"]').html("<?php echo lang('edit_deposit'); ?>")
                                    //    $('.modal-title').html("<?php echo lang('edit_deposit'); ?>");
                                    ...
                                    $.ajax({
                                        ...
                                        if (response.deposit.deposit_type != 'Card') {
                                            $('#deposit-form').find('[name="id"]').val(response.deposit.id).end()
                                            $('#deposit-form').find('[name="patient"]').val(response.deposit.patient).end()
                                            $('#deposit-form').find('[name="payment_id"]').val(response.deposit.payment_id).trigger("change");
                                            $('#deposit-form').find('[name="date"]').val(response.deposit.date).end()
                                            $('#deposit-form').find('[name="deposited_amount"]').val(response.deposit.deposited_amount).end()
...
<script type="text/javascript">
    ...
    function openModal() {
        ...
        $('#myModal').find('[class="modal-title"]').html("<?php echo lang('add_deposit'); ?>")
        $('#myModal').modal('show');
    }
</script>
<script>
