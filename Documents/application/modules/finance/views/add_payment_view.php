                                ...
                                <div class="">
                                    ...
                                    <form role="form" id="editPaymentForm" class="clearfix" action="finance/addPayment" method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="col-md-5 row">
                                            ...
                                            <div class="col-md-12 payment pad_bot">
                                                <label for="exampleInputEmail1"> <?php echo lang('refd_by_doctor'); ?>kkk</label>
                                            ...
                                            <div class="col-md-12  pad_bot">
                                                <div class="btn btn-info row pull-right" id="custom_add"> Add Custom</div>
                                            </div>
...
<div class="modal fade" id="custompayment" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button id="c_close" type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h4 class="modal-title">Add Custom Payment</h4>
            </div>
            <div class="modal-body">
                <form role="form" action="patient/addNew?redirect=payment" method="post" enctype="multipart/form-data">
                    <input type="hidden" value="0" name="sCount" id="sCount" />
                    <div class="row" id="billAdder">
                    </div>
                    <div style="text-align:center">
                        <div id="addBill" class="btn green btn-xs">
                            <i class="fa fa-plus-circle"></i> Add Payment
                        </div>
                    </div>
                    <div style="text-align:center">
                        <p id="c_error" style="color:red; font-weight:bold; padding:10px 0;"></p>
                    </div>
                    <section class="">
                        <div onclick="addBill()" class="btn btn-info" id="c_submit">Submit</div>
                        <img src="common/img/ajldr.gif" style="display:none" id="c_loader"/>
                    </section>
                </form>
...
<script>
    $(document).ready(function(){
        $("#custom_add").click(function(){
            ...
            console.warn("click function working")
        })
    })    
</script>
...
<script type="text/javascript">
    $("#addBill").click(function(){
        num=document.getElementById("billAdder").children.length+1;
        //add a new symp
        var con="<div>"+
            "<div class='col-lg-7'>"+
                "<label for='exampleInputEmail1'>Bill</label>"+
                "<input type='text' class='form-control' id='bill_"+num+"' value='' placeholder=''>"+
            "</div>"+
            "<div class='col-lg-5'>"+
                "<label for='exampleInputEmail1'>Amount</label>"+
                "<input type='text' class='form-control' id='amount_"+num+"'  value='' placeholder=''>"+
            "</div>"+
        "</div>"
        $("#billAdder").append(con)
        document.getElementById("sCount").value=document.getElementById("billAdder").children.length
    })
</script>

<script>
    function addBill(){
        //get the total number of custom bills added
        var total=document.getElementById("billAdder").children.length;
        //loop through all the bills
        var i=0,
        ajaxSend="count="+total;
        _("c_error").innerHTML="";
        _("c_submit").style.display="none";
        _("c_loader").style.display="inline-block";
        for(i; i<total; i++){
            var e=i+1;
            var bill_id="bill_"+e,
            am_id="amount_"+e,
            bill=_(bill_id).value,
            amount=_(am_id).value;
            //add the ajax send bill in a variable
            ajaxSend+="&"+bill_id+"="+bill+"&"+am_id+"="+amount
        }
        //add the listener here and listen for errors
        var ajax=new ajaxObj("POST","finance/addCustomBill");
        ajax.onreadystatechange=function(){
            if(ajaxReturn(ajax)==true){
                var get=ajax.responseText.split("|");
                if(get[0] == "ok"){
                    setTimeout(() => {
                        _("c_close").click()
                    }, 3000);
                    _("c_error").innerHTML=get[1];
                    _("c_error").style.color="green";

                }else{
                    _("c_error").innerHTML=ajax.responseText;
                    _("c_submit").style.display="inline";
                    _("c_error").style.color="red";
                    _("c_loader").style.display="none";
                }
            }
        }
