...
class Pharmacy extends MX_Controller {
    ...
    public function addPaymentView() {
        ...
        if($this->input->get("pid") !== null){
            ...
            $data['prescription'] = $this->prescription_model->getPrescriptionById($this->input->get("pid"));
        }
        else if($this->input->get("patient") !== null){
            //get prescription list
            $data['patient_info'] = $this->patient_model->getPatientById($this->input->get("patient"));
        }
        $this->load->view('home/dashboard', $data); // just the header file
    ...
    public function addPayment() {
        ...
        foreach ($item_quantity_array as $key => $value) {
            ...
            if ($current_stock < $qty) {
                $this->session->set_flashdata('feedback', 'Unsufficient Quantity selected for Medicine ' . $current_medicine->name);
                redirect('finance/pharmacy/addPaymentView?pid='.$pid);
            }
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $p_id=$this->pharmacist_model->getPharmacistByIonUserId($user_id)->id;
            ...
            if (empty($id)) {
                ...
                $this->prescription_model->updatePrescription($pid, $uD);
