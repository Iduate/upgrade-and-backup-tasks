...
class Medicine extends MX_Controller {
    ...
    function __construct() {
        ...
        if (!$this->ion_auth->in_group(array('admin', 'Pharmacist', 'Doctor','Nurse'))) {
...
// Get users
    ...
    //custom add medicine
    public function addfMedicine() {
        $file_path="./uploads/medicine.csv";
        $i=0;
        $e=1;
        if (($open = fopen($file_path, "r")) !== FALSE) {
            while (($dat = fgetcsv($open, 1000, ",")) !== FALSE) {
                    $spn=$this->finance_model->getSponsor("19");
                    foreach($spn as $n){
                        $hmo_id=$n->id;
                        $pay_id="mdc".$dat[0];
                        $price=$dat[2];
