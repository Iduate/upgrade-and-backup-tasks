...
class Finance_model extends CI_model {
    ...
    function addInsurancePrice($data) {
        $this->db->insert('hmo_price', $data);
    }
    ...
    function getHMOPriceBySearch($search,$id) {
        $this->db->order_by('id', 'desc');
        $query = $this->db->select('*')
                ->from('hmo_price')
                ->where('hmo_id',$id)
                ->where("(hmo_id LIKE '%" . $search . "%' OR name LIKE '%" . $search . "%' OR price LIKE '%" . $search . "%')", NULL, FALSE)
                ->get();

        return $query->result();
    }

    function getHMOPrice($id) {
        $this->db->where('hmo_id',$id);
        $this->db->order_by('id', 'desc');
        $query = $this->db->get('hmo_price');
        return $query->result();
    }

    

    function getHMOCategoryPrice($hmo_id,$cat_id) {
        $this->db->where('hmo_id',$hmo_id);
        $this->db->where('payment_category',$cat_id);
        $query = $this->db->get('hmo_price');
        return $query->row();
    }

    function getHMOPriceByLimitBySearch($limit, $start,$search,$id) {
        $this->db->order_by('id', 'desc');
    ...
    function getHMOPriceByLimit($limit, $start,$id) {
        $this->db->order_by('id', 'desc');
        $this->db->where('hmo_id',$id);
        $this->db->limit($limit, $start);
        $query = $this->db->get('hmo_price');
    ...
    function getHMOBySearch($search) {
        $this->db->order_by('id', 'desc');
        $query = $this->db->select('*')
                ->from('hmo')
                ->where('hospital_id', $this->session->userdata('hospital_id'))
    ...
    function getHMO() {
        $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
        $this->db->order_by('id', 'desc');
        $query = $this->db->get('hmo');
        return $query->result();
    }

    function getHMOById($id) {
        $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
        $this->db->where('id', $id);
        $query = $this->db->get('hmo');
        return $query->row();
    ...
    function getHMOByLimitBySearch($limit, $start,$search) {
        $this->db->order_by('id', 'desc');
        $this->db->limit($limit, $start);
        $query = $this->db->select('*')
                ->from('hmo')
    ...
    function getHMOByLimit($limit, $start) {
        $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
        $this->db->order_by('id', 'desc');
        $this->db->limit($limit, $start);
        $query = $this->db->get('hmo');
    ...
    function getSponsorBySearch($id, $search) {
        $this->db->order_by('id', 'desc');
        $query = $this->db->select('*')
                ->from('hmo_sponsor')
                ->where('insurance_id', $id)
                ->where("(name LIKE '%" . $search . "%' OR name LIKE '%" . $search . "%')", NULL, FALSE)
                ->get();
    ...
    function getSponsorByLimitBySearch($id, $limit, $start,$search) {
        $this->db->order_by('id', 'desc');
        $this->db->limit($limit, $start);
        $query = $this->db->select('*')
                ->from('hmo_sponsor')
                ->where('insurance_id', $id)
                ->where("(name LIKE '%" . $search . "%')", NULL, FALSE)
                ->get();

        return $query->result();
    }

    function getSponsorByLimit($id, $limit, $start) {
        $this->db->where('insurance_id', $id);
        $this->db->order_by('id', 'desc');
        $this->db->limit($limit, $start);
        $query = $this->db->get('hmo_sponsor');
    ...
    function addInsurance($data) {
        $data1 = array('hospital_id' => $this->session->userdata('hospital_id'));
        $data2 = array_merge($data, $data1);
        $this->db->insert('hmo', $data2);
        return $this->db->insert_id();
    }

    function addSponsor($data) {
        $this->db->insert('hmo_sponsor', $data);
    }
    ...
    function updateInsurance($id, $data) {
        $this->db->where('id', $id);
        $this->db->update('hmo', $data);
    }

    function updateInsurancePrice($id, $data) {
        $this->db->where('id', $id);
        $this->db->update('hmo_price', $data);
    }
    ...
    function getPaymentCategoryByOperation($id) {
        $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
    ...
    function getCategoryInfo($searchTerm) {
        if (!empty($searchTerm)) {
            $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
            $this->db->select('*');
        ...
        foreach ($users as $user) {
            $data[] = array("id" => $user['id'], "text" => $user['category'] . ' (Price: ' . $user['c_price'] . ')');
        }
        return $data;
    }

    function getInsuranceInfo($searchTerm,$type) {
        if (!empty($searchTerm)) {
            $this->db->select('*');
            $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
            $this->db->where('type', $type);
            $this->db->where("name like '%" . $searchTerm . "%' OR id like '%" . $searchTerm . "%'");
            $fetched_records = $this->db->get('hmo');
            $users = $fetched_records->result_array();
        } else {
            $this->db->select('*');
            $this->db->limit(10);
    ...
    function getAllInsuranceInfo($searchTerm) {
        if (!empty($searchTerm)) {
            $this->db->select('*');
            $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
            $this->db->where("name like '%" . $searchTerm . "%' OR id like '%" . $searchTerm . "%'");
    ...
    function getSponsorInfo($searchTerm,$type) {
        if (!empty($searchTerm)) {
            $this->db->select('*');
            $this->db->where('insurance_id', $type);
            $this->db->where("name like '%" . $searchTerm . "%' OR id like '%" . $searchTerm . "%'");
            $fetched_records = $this->db->get('hmo_sponsor');
            $users = $fetched_records->result_array();
        } else {
            $this->db->select('*');
            $this->db->limit(10);
    ...
    function getAllSponsorInfo($searchTerm) {
        if (!empty($searchTerm)) {
            $this->db->select('*');
            $this->db->where("name like '%" . $searchTerm . "%' OR id like '%" . $searchTerm . "%'");
            $fetched_records = $this->db->get('hmo_sponsor');
    ...
    function getPatientClear($id){
        $e=time();
        $this->db->where("patient",$id);
        $this->db->where("end >", $e);
        $query = $this->db->get("patient_clear");
        if($query->num_rows() >0){
            return true;
        }else{
            return false;
    ...
    function patientClear($id,$d){
        //patient	start	end	cleared_by	date
        $e=time()+$d;
        $data=array(
            "patient"=>$id,
            "start" => time(),
            "end" => $e,
            "cleared_by" =>$this->ion_auth->get_user_id(),
            "date" =>time()
        );
        if($this->db->insert('patient_clear', $data)){
            return true;
        }else{
            return false;
        }
