...
<!--main content start-->
<style>
.pro{display:none; position: fixed; width: 100%; height: 100%; background: rgba(255,255,255,.8);
z-index: 99999999999999999999999; padding: 300px 100px; font-weight: bold; font-size:20px; text-align: center;}
</style>
<section id="main-content">
    <section class="wrapper site-min-height">
        <div class="pro">Processing... please wait</div>
        <!-- page start-->
                            ...
                            <div class="">
                                ...
                                <?php
                                    ...
                                    if(!empty($request->doctor)){ 
                                        ...
                                        $name="po";
                                        echo "<input type='hidden' name='patient' value='".$rp->id."' />";
                                    }
                                ?>
                                    <label for="exampleInputEmail1"><?php echo lang('patient'); ?></label>
                                    <select <?php echo $dis;?> class="form-control m-bot15 pos_select" id="pos_select" name="<?php echo $name;?>" onchange='pcheck()'> 
                                        ...
                                        <?php }else if(!empty($request->patient_id)){ ?>
                                            <option value="<?php echo $rp->id; ?>" selected="selected"><?php echo $rp->name; ?> - <?php echo $rp->id; ?></option>
                                            <?php
                                                }
                                            ?>
                                    </select>
                                ...
                                <div class="col-md-8 panel"> 
                                  <?php
                                    if(!empty($request->doctor)){
                                        echo "<input type='hidden' name='rid' value='$request->id' />";
                                    }?>
                                </div>
                                ...
                                <?php
                                    ...
                                    if(!empty($request->doctor)){ 
                                        $dis="disabled='disabled'";
                                        $name="po";
                                        echo "<input type='hidden' name='doctor' value='".$rp_doc->id."' />";
                                    }
                                ?>
                                    <label for="exampleInputEmail1"> <?php echo lang('refd_by_doctor'); ?></label> 
                                    <select <?php echo $dis;?> class="form-control m-bot15  add_doctor" id="add_doctor" name="doctor" value=''>  
                                      <?php if (!empty($lab_single->doctor)) { ?>
                                        ...
                                        <?php }else if(!empty($request->doctor)){ ?>
                                            <option value="<?php echo $rp_doc->id; ?>" selected="selected"><?php echo $rp_doc->name; ?> - <?php echo $rp_doc->id; ?></option>  
                                        <?php } ?>
                                    </select>
                                </div>
        ...
        <section class="col-md-12" style="display:none">
            <header class="panel-heading">
                ...
                <div class="col-md-4 no-print pull-right"> 
                    <a href="lab/addLabView"  style="display:none">
...
<script>
    ...
    function pcheck(){
        var select= document.getElementById("pos_select");
        ...
        $.ajax({
            url: 'patient/getPatientBill?id=' + patient,
            method: 'GET',
            data: '',
            dataType: 'json',
        }).success(function (response) {
            if(response.pay!=null){
                $(".pro").html(response.pay);
                return;
            }else{
                //ask the question
